...
function StatCard({
  ...
  return (
    <Link
      to={to}
      className="block rounded-[1.5rem] focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/60"
    >
      <article
        className={`h-full rounded-[1.5rem] border ${borderClassName} bg-white dark:bg-slate-700 p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md`}
      >
        <div className="flex items-center gap-2.5">
          <div
            className={`shrink-0 rounded-2xl p-2.5 ${iconWrapperClassName}`}
          >
            {icon}
          </div>
...
export default function GeneralPanel() {
  return (
    <section className="flex w-full flex-col gap-6 rounded-[2rem] border border-gray-200 dark:border-slate-600 bg-white/70 dark:bg-slate-700/80 p-5 shadow-[0_32px_80px_-52px_rgba(15,23,42,0.45)] backdrop-blur-sm sm:p-7">
