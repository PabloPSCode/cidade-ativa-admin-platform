...
import { ITutor } from "@/interfaces/dtos/Tutor";
...
export function TutorsTable({
  ...
  const [sortedTutors, setSortedTutors] = useState<ITutor[]>([]);
  ...
  const pages = Array.from(
    { length: Math.ceil(sortedTutors.length / itemsPerPage) },
    (_, idx) => idx + 1
  );
  ...
  const currentTableData = useCallback(() => {
    ...
    setTableData(sortedTutors.slice(startIndex, endIndex) as never);
  }, [itemsPerPage, page, sortedTutors]);

  useEffect(() => {
    setSortedTutors(tutors);
  }, [tutors]);
  ...
  const handleSort = (propRef: string, sortType: "asc" | "desc") => {
    const sortedData = sortItems([...tutors], propRef, sortType);
    setSortedTutors(sortedData);
    setPage(1);
  };
      ...
      <CardBody className="overflow-scroll p-0 rounded-lg w-full">
        <table className="w-full table-auto text-left">
          ...
          <tbody className="w-[90%] lg:w-full">
            {tableData.map(({ name, bio, cpf, email, phone }, index) => {
              ...
              return (
                ...
                >
                  ...
                  <td className={classes}>
                    <Text
                      content={collapseLongString(bio, MAX_BIO_LENGTH)}
                      className="block overflow-hidden text-ellipsis whitespace-nowrap text-[11px] md:text[12px] lg:text-sm ml-2 lg:ml-4 text-gray-700 dark:text-gray-300"
                    />
                  </td>
                  <td className={classes}>
                    <Tooltip
                      content="Editar dados do tutor"
                      className="hidden lg:flex"
                    >
                      <IconButton
                        variant="text"
                        onClick={onUpdateTutor}
              ...
              <div className="flex flex-row">
                ...
                {pages
                  ...
                  .map((tutorPage) => (
                    <IconButton
                      key={tutorPage}
                      variant={page === tutorPage ? "outlined" : "text"}
                      size="sm"
                      onClick={() => setPage(tutorPage)}
